...
function activitystatus_cm_info_view(cm_info $cm) {
    ...
    if ($completioninfo->is_tracked_user($USER->id) || $can_edit) {
        ...
        $itemsperrow = activitystatus_load_items_per_row($cm->instance);
        $width = (int) (12 / $itemsperrow); // Bootstrap
        ...
        $content .= html_writer::start_div('widgetcontents row d-flex flex-wrap');
        foreach ($iconsorder as $key => $order) {
            ...
            if (isset($item->module)) {
                ...
                $content .= html_writer::start_div('modcontainer d-flex col-sm-' . $width);
                ...
                if (isset($item->id)) { // Course may have been removed from completion criteria.
                    ...
                    $content .= html_writer::start_div('modcontainer col-sm-' . $width);
